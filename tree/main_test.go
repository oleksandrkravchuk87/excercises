...
func Build(records []Record) (*Node, error) {
	...
	if records[0].ID != 0 || records[0].Parent != 0 {
		return nil, errors.New("root err")
	}
	...
	for i := 1; i < len(records); i++ {
		if records[i].ID != i {
			return nil, fmt.Errorf("missing record with ID %d", i+1)
		}

		if records[i].Parent >= records[i].ID {
			return nil, errors.New("parent ID is not less? then ID")
		}

		n := &Node{ID: records[i].ID}
		parent := nodes[records[i].Parent]
		if parent.Children == nil {
			parent.Children = []*Node{}
		}
		parent.Children = append(parent.Children, n)
		nodes[records[i].ID] = n
